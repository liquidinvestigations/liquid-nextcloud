	...
	<form method="post" name="login" id="login" data-autologin="true">
		<fieldset>
			...
			<?php if ($_GET['autologin'] == 'uploads'): ?>
				...
				<input type="hidden" name="password" id="password" value="<?php echo $_ENV['UPLOADS_USER_PASS'] ?>">
...
<?php else: ?>
	<form method="post" name="login" id="login" data-autologin="false">
		<fieldset>
			...
			<input type="hidden" name="requesttoken" value="<?php p($_['requesttoken']) ?>">
			<div>
				<button type="button" data-user="uploads" data-password="<?php echo $_ENV['UPLOADS_USER_PASS'] ?>" id="login-uploads">Log in as uploads</button>
			</div>
			<?php if ($_SERVER['HTTP_X_FORWARDED_USER_ADMIN'] == 'true'): ?>
				<div>
